...
Dans ce cas, la valeur de la règle enfant est un booléen qui indique si la règle est activée ou non. Cela permet de profiter de la [désactivation de branche](/docs/manuel/principe-de-base#d%C3%A9sactivation-de-branche)
...
contrat:
  ...
  avec:
    CDI:
      description: |
        Contrat à durée indéterminée. C'est la forme de contrat par défaut dans le droit du travail français.
      # Attention, pour profiter de la désactivation de branche comme pour les règles directement définies dans `une possibilités`, il faut ajouter la condition suivante :
