...
Un remplacement remplace **les références** à une règle, et non la règle elle-même. 
Dans l'exemple ci-dessus, la règle `temps de préparation` **n'est pas modifiée** : ce sont ses références qui le sont.

Les remplacements sont implémentés sous forme d'une substitution de variable.
:::

### Remplacement défini dans une règle non applicable

Si le remplacement est défini dans une règle dont la valeur est `non` ou `non applicable`, alors ce dernier n'est pas appliqué : 
...
résultat: a # vaut 1 car non remplacé
```
...
1. Le remplacement avec la `priorité` la plus haute est utilisée
  ...
  résultat: a 
  # vaut 5 car le remplacement défini dans `b` a une priorité 
  # plus élevée que celui défini dans `c` (par défaut à zéro)
  ```
...
2. Sinon, c'est le nom de la règle de définition qui est utilisé (comparaison lexicographique)
  ...
  résultat: a 
  # vaut 10 car `b . c` est situé après `b` dans l'ordre alphabétique
  ``` 
