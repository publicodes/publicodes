...
Pour qu'une valeur soit interprétée comme une chaîne de caractères littérale et non comme
une référence à une autre règle, il faut que la chaîne de caractères soit
encadrée d'apostrophes `'` ou de guillemets `"`. Dans certaines situations,
notamment à la définition de la chaîne de caractères, ils devront être échappés
pour qu'ils ne soient pas ignorés car considérés comme des éléments de la syntaxe Yaml.

L'exemple suivant montre plusieurs façons de définir des chaînes de caractères
littérales :
