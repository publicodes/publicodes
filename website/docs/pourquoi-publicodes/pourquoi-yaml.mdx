...
Ça oblige à utiliser des fonctions pour tout calcul : impossible d'écrire un code ad-hoc. Il faut écrire, nommer, spécifier, implémenter et documenter une fonction, que l'on appelle des mécanismes. 
...
Il est fort probable que la liberté de ne pas définir des fonctions et d'utiliser du code ad-hoc soit fréquemment exploitée par les développeurs. L'écriture est plus rapide... mais la relecture peut devenir très pénible. Le code n'est plus une représentation de la loi, mais simplement une implémentation dans le seul but de faire des calculs. Il alors aussi très difficile de le lire avec un programme pour en faire autre chose.
