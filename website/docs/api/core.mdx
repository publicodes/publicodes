...
- `options` : les options de configuration du moteur
  ...
  - `getUnitKey` : permet de spécifier une fonction de _normalisation_ des unités.
    Par défaut, les unités ne sont pas _normalisées_.
...
- `options` : Paramètres optionnels sous forme d'objet
  - `keepPreviousSituation` : par défaut, l'appel à `setSituation` écrase la situation précedemment renseignée. Pour faire une mise à jour en gardant l'état en cours, il suffit de mettre ce paramètre à `true`.
  - `filterSituation` : par défaut, si la situation renseignée contient :
    - une règle absent de la base de règle
    - une "réponse" donnée via un mécanisme comme "une possibilité" n'est pas une option de réponse dans la base de règle alors, le moteur renvoie une erreur. Cette option permet d'éviter au moteur de casser. Des warnings sont alors levés dans la console.
...
Permet de récuperer la situation courante du moteur, notamment dans le cas ou la situation initiale a été filtrée avec l'option `filterSituation`.
