...
:::caution Ajouter des conversions automatiques

Il n'est pour l'instant pas possible d'ajouter des conversions. Cela sera possible dans une prochaine version de publicodes. [voir le ticket correspondant](https://github.com/betagouv/publicodes/issues/340)
...
salaire exemple:
	valeur: 2 * SMIC
	unité: €/an
	note: Ici le salaire est présenté sous forme annuelle. La conversion est automatique.
```

Ce mécanisme `unité` peut aussi être utilisé lors de l'évaluation d'une règle avec l'API Javascript <!-- TODO ajouter lien interne --> :

```javascript
const salaireExemple = engine.evaluate({
	valeur: '2 * SMIC',
	unité: '€/an',
})
```
...
[TODO](https://github.com/betagouv/publicodes/issues/340)
