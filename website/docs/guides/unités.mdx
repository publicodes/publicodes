...
Il est possible de spécifier un pourcentage comme unité. Le pourcentage est une unité qui se comporte d'une manière particulière :

- L'addition d'un nombre avec un pourcentage revient à ajouter le pourcentage à ce nombre.
- De même pour la soustraction.

```publicodes
prix TTC: 10€ + 20%
prix avec réduction: prix TTC - 10%
```
