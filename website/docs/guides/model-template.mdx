...
Avec le nouveau [système d'import de règles](../tools/imports), il est possible d'implémenter son
modèle Publicodes dans un dépôt dédié et de le publier sur NPM. Cela permet de
faciliter la réutilisation de votre modèle dans différents projets.

## Fonctionnalités

Ce template permet de setup un dépôt avec les fonctionnalités suivantes :
...
La suite des instruction est disponible dans le README du dépôt.
