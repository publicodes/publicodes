...
### _method_ `engine.evaluateApplicability(expression)`

Cette méthode permet déterminer l'applicabilité d'une expression. Elle est utile
pour l'affichage de formulaires où l'on veut afficher certaines sections de
manière conditionnelle.

Cette méthode réalise seulement les calculs utiles pour déterminer
l'applicabilité de l'expression, et non ceux utiles pour le calcul de sa valeur
comme avec `engine.evaluate`. Les `missingVariables` retournées seront
uniquement celles utiles pour déterminer l'applicabilité.
...
- `isApplicable: boolean` applicabilité de l'expression en entrée
- `missingVariables` les règles utilisés pour déterminer l'applicabilité de l'expression
