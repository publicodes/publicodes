...
<div>
    ...
    <div class="mt-8 grid max-w-lg gap-5 lg:max-w-none lg:grid-cols-3">
    ...
    <ul class="divide-y divide-gray-200">
        {#each data.blogPosts as { path, metadata: { title, description, date, tags } }}
            <PostListItem url={'blog' + path} {title} {description} {date} {tags} />
        {/each}
    </ul>
</div>
