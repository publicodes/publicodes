...
`,
			...
			expect(engine.evaluate('a . a').possibleChoices[0].dottedName).to.eql(
				'a . b . a',
			)
		})

		test('Is undefined by default', function () {
			const engine = engineFromYaml(
				`
a:
  une possibilité:
...
`,
			...
			expect(engine.evaluate('a').nodeValue).to.eql(undefined)
		})
