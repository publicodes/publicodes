...
export function parseChoixPossibles(
	choices: NonNullable<RulePossibilities>,
	context: Context,
): [
...
Déplacer une des règles suivantes dans un autre élément du tableau : ${rules.map(
			...
			const ruleName = rules[0][0]
			const rule = rules[0][1] ?? {}
			rule.valeur = {
				et: [
					{ 'est défini': context.dottedName },
					`${context.dottedName} = '${ruleName}'`,
				],
			}
			avec[ruleName] = rule
...
Les choix possibles doivent être des constantes (nombre, texte, date, etc.) ou des références à une règle existante.`,
		...
		if (
			'type' in parsedChoice &&
			parsedChoice.type === 'number' &&
			'unit' in parsedChoice
		) {
			const newUnit = serializeUnit(parsedChoice.unit)
			if (unit && newUnit !== unit) {
				throw new PublicodesError(
					'SyntaxError',
					`Les choix possibles doivent être homogènes en unité.
Le choix "${choice}" n'a pas la même unité que les autres choix.`,
					context,
				)
			} else {
				unit = newUnit
