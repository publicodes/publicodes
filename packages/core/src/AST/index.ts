...
const traverseRuleNode: TraverseFunction<'rule'> = (fn, node) => {
	...
	copy.explanation = {
		...
		possibilities:
			node.explanation.possibilities &&
			(fn(node.explanation.possibilities) as UnePossibilitéNode),
	}
	return copy
}
...
const traverseUnePossibilitéNode: TraverseFunction<'une possibilité'> = (
	...
	const copy = weakCopyObj(node)
	copy.explanation = node.explanation.map(fn) as Array<ReferenceNode>
	if (copy.type === 'reference') {
		copy.notApplicable = copy.notApplicable?.map(fn) as any
	}
	return copy
}
