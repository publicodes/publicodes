...
import parse, { mecanismKeys, valueMecanismKeys } from './parse'
...
au niveau supérieur.`,
	...
	if (!privateRule && !dottedName.endsWith('$SITUATION')) {
		...
		// If there is no valeur mecanism to the rule then the rule should be listed as a missingVariables
		if (valueMecanismKeys.every((key) => !(key in ruleValue))) {
			const valeur = ruleValue['par défaut']
