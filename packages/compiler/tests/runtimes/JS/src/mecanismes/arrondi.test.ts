...
`
		...
		test('arrondi à 0 décimales', () => {
			expect(value(a.evaluate({ b: 0 }))).toBe(12)
		})

		test('arrondi à 5 décimales', () => {
			expect(value(a.evaluate({ b: 5 }))).toBe(12.458)
			expect(a.unit).toBe('%')
		})

		test('arrondi à -1 décimales', () => {
			expect(value(a.evaluate({ b: -1 }))).toBe(10)
		})
	})
...
`
		...
		test('arrondi à 0 (erreur)', () => {
			expect(() => value(rules.a.evaluate({ b: 0 }))).toThrowError(
				'Rounding error',
			)
		})

		test('arrondi à 5 près', () => {
			expect(value(rules.a.evaluate({ b: 5 }))).toBe(15)
		})

		test('arrondi à 10 près', () => {
			expect(value(rules.a.evaluate({ b: 10 }))).toBe(10)
		})

		test('arrondi à 0.005 près', () => {
			expect(value(rules.a.evaluate({ b: 0.005 }))).toBe(13.46)
		})
	})
...
`
			expect(value(rules.a.evaluate())).toBe(35.465729905)
		})
...
`
			expect(
				p.isNotApplicable(
					value(engine["arrondi d'une valeur non applicable"].evaluate()),
				),
			).toBeTrue()

			expect(value(engine['arrondi non applicable'].evaluate())).toBe(13.45)
		})
...
test arrondi:
  arrondi: oui
`
			const result = value(engine['test arrondi'].evaluate())
...
`
		...
		expect(value(montant.evaluate())).toBe(150)
	})
...
`
		...
		expect(value(a.evaluate({ a: 1.4 }))).toBe(1)
	})
})
