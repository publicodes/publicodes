...
`
	...
	test('entier', () => {
		expect(value(r.entier.evaluate())).toBe(5)
		expect(r.entier.type).toBe('number')
	})

	test('nombre décimal', () => {
		expect(value(r.décimal.evaluate())).toBe(5.4)
		expect(r.décimal.type).toBe('number')
	})

	test('date', () => {
		expect(value(r.date.evaluate())).toEqual(new Date('2025-12-03'))
		expect(r.date.type).toBe('date')
	})

	test('booléen', () => {
		expect(value(r.booléen.evaluate())).toBe(true)
		expect(r.booléen.type).toBe('boolean')
	})

	test('chaine', () => {
		expect(value(r.chaine.evaluate())).toBe('Salut le monde')
