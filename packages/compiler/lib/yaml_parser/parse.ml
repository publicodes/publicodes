...
let print_token =
  ...
  | Alias _ -> "Alias"
  | Scalar _ -> "Scalar"
  | Stream_start _ -> "Stream_start"
  | Document_start _ -> "Document_start"
  | Document_end _ -> "Document_end"
  | Mapping_start _ -> "Mapping_start"
  | Mapping_end -> "Mapping_end"
  | Stream_end -> "Stream_end"
  | Sequence_start _ -> "Sequence_start"
  | Sequence_end -> "Sequence_end"
  | Nothing -> "Nothing"

let parse (filename : string) (content : string) : (yaml, Log.t) result =
  ...
  let transform_error ?(pos = With_pos.beginning_of_file filename) result =
    ...
    | Error (`Msg err) ->
        ...
        in
        Error (With_pos.mk pos Log.{ kind = Yaml; level = Err; message; hint })
  in
  let%bind parser = parser content |> transform_error in
  let%bind first_token = do_parse parser |> transform_error in
  let current_token = ref first_token in

  (* Create an error message based on the position of the last parsed token *)
  let error message =
    let _, pos = !current_token in
    Error
      (With_pos.mk (pos_from_mark pos)
         Log.{ kind = Yaml; level = Err; message; hint = None })
  in
  ...
  let rec parse_stream () =
    ...
    | Event.Stream_start _ -> (
        let%bind result = parse_document () in
        let%bind stream_end, _ = next () in
        match stream_end with
        | Event.Stream_end -> Ok result
        | token -> unexpected_token_error token "the end of stream")
    | token -> unexpected_token_error token "the beginning of stream"
  and parse_document () =
    let%bind document_start, _ = next () in
    match document_start with
    | Event.Document_start _ -> (
        let%bind _ = next () in
        let%bind result = parse_node () in
        let%bind document_end, _ = next () in
        match document_end with
        | Event.Document_end _ -> Ok result
        | token -> unexpected_token_error token "the end of file")
    | token -> unexpected_token_error token "the beginning of file"
  and parse_node () =
    let event, pos = !current_token in
    match event with
    | Event.Scalar scalar ->
        let pos = pos_from_mark pos in
        Ok (`Scalar (make_scalar pos scalar))
    | Event.Sequence_start _ -> parse_sequence []
    | Event.Mapping_start _ -> parse_mapping []
    | Event.Alias _ ->
        error "Il n'est pas possible d'utiliser un alias avec Publicodes"
    | Event.Nothing -> error "Le fichier est vide"
    | _ -> failwith "Internal error"
  and parse_sequence (seq : sequence) =
    let%bind event = next () in
    match event with
    | Event.Sequence_end, _ -> Ok (`A (List.rev seq))
    | _ ->
        let%bind node = parse_node () in
        parse_sequence (node :: seq)
  and parse_mapping (mapping : mapping) =
    let%bind event, _ = next () in
    match event with
    | Event.Mapping_end -> Ok (`O (List.rev mapping))
    | _ ->
        let%bind key = parse_key () in
        let%bind _ = next () in
        let%bind value = parse_node () in
  ...
  and parse_key () =
    ...
    | Event.Scalar scalar ->
        let pos = pos_from_mark pos in
        Ok (make_scalar pos scalar)
    | token -> unexpected_token_error token "une chaine de caractères"
  in
  parse_stream ()
