...
let%test_unit "do not disambiguate complete reference" =
  ...
  let ref = ["rule 2"] in
  [%test_eq: string list option] (resolve ~rules ~current ref) (Some ref) ;
...
let%test_unit "disambiguate child rule" =
  ...
  let current = ["a"] in
  let ref = ["b"] in
  [%test_eq: string list option] (resolve ~rules ~current ref) (Some ["a"; "b"]) ;
...
let%test_unit "disambiguate parent rule" =
  let rules = Dotted_name.Set.of_list [["a"]; ["a"; "b"]; ["a"; "b"; "c"]] in
  let current = ["a"; "b"; "c"] in
  let ref = ["b"] in
  [%test_eq: string list option] (resolve ~rules ~current ref) (Some ["a"; "b"])

let%test_unit "disambiguate child rule first" =
  let rules = Dotted_name.Set.of_list [["a"]; ["a"; "a"]; ["a"; "a"; "a"]] in
  let current = ["a"; "a"] in
  let ref = ["a"] in
  [%test_eq: string list option]
    (resolve ~rules ~current ref)
    (Some ["a"; "a"; "a"])
...
let%test_unit "disambiguate current rule last" =
  ...
  let current = ["a"; "a"] in
  let ref = ["a"] in
  [%test_eq: string list option] (resolve ~rules ~current ref) (Some ["a"])
