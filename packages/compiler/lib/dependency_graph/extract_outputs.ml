...
let extract_outputs ~(ast : 'a Shared_ast.t) ~(eval_tree : Hashed_tree.t)
  ...
  List.iter ast ~f:(fun rule_def ->
      ...
      if not (Shared_ast.has_value rule_def) then
        G.add_edge graph rule_name rule_name ) ;
  ...
  let rec chop_contexts ?(acc = []) graph from =
    let rule_def = Eval_tree.get_value eval_tree from in
    let contexts = Eval_tree.get_contexts rule_def in
    let acc = acc @ contexts in
    List.iter acc ~f:(fun (target, _) ->
        Rule_graph.remove_edge graph from target ) ;
  ...
  let warnings =
    List.filter_map rules ~f:(fun {rule_name; typ; _} ->
        ...
        | Some _ ->
            None )
  in
  if warn_types then return ~logs:warnings rules else return rules
