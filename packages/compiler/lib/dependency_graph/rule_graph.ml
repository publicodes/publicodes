...
let output_dot
    ?(file_path =
      "/home/emile/Dev/publicodes/publicodes/packages/compiler/rule_graph.dot")
    graph =
...
and find_references_in_value (only_in_chainable : bool)
  ...
  | Shared_ast.Expr expr ->
      find_references_in_expr expr
      |> List.map ~f:(fun ref_name ->
          let ref_rule = Pos.value ref_name in
          (ref_rule, context_stack) )
  | Shared_ast.Value v ->
      find_references v
...
let mk ast =
  ...
  (let rec add_rule_dependencies
     ...
     if List.exists !visited_node ~f:(Rule_vertex.equal current_node) then
       ...
       in
       List.iter refs ~f:(fun referenced_node ->
           let ref_name, ref_ctx_stack = referenced_node in
           G.add_vertex graph referenced_node ;
           let pos =
             (* FIXME: correclty handle pos, is it really used? *)
             Pos.dummy
           in
           let edge = (current_node, pos, referenced_node) in
