...
module Unit = struct
  ...
  type naked_t = Concrete of Units.t | Any of Any.t | Inv of t | Elem of t
  ...
  (* Normalize unit have only one Concrete unit, one Inv and no Elem  *)
...
let to_str = function
  | Number u -> (
    match Unit.normalize u with
    | {concrete; elem= []; inv= []} ->
        Format.asprintf "en « %a »" Units.pp concrete
    | _ ->
        "Un nombre (unité inconnue)" )
  | Literal String ->
      "un texte"
  | Literal Bool ->
      "un booléen (oui / non)"
...
let unify (t1 : t) (t2 : t) =
    ...
    fatal_error ~pos:(Pos.pos typ1) ~kind:`Type ~code
      ~labels:
        [ Pos.mk ~pos:(Pos.pos typ1)
            (Format.sprintf "est %s" (to_str (Pos.value typ1)))
        ; Pos.mk ~pos:(Pos.pos typ2)
            (Format.sprintf "est %s" (to_str (Pos.value typ2))) ]
      message
  in
  ...
  | Number n1, Number n2 -> (
    try
      let _ = Unit.unify n1 n2 in
      return t1
    with Failure _ -> error_typ_mismatch )
