...
let publicodes_module path =
  if not (Stdlib.Sys.file_exists path) then None
  else if not (Stdlib.Sys.is_directory path) then None
  else
    let dir = Fpath.of_string path |> Stdlib.Result.get_ok in
    let files =
      Stdlib.Sys.readdir path |> List.of_array
