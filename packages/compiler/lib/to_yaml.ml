...
(* TODO: move/rename when change to templates lands *)

let rec to_value ({value; pos; meta= {typ; _}} : Hashed_tree.value) =
  let _type, value =
    match value with
    ...
    | Shared.Eval_tree.Const Undefined ->
        ("constante", `String "undefined")
    | Shared.Eval_tree.Const Null ->
        ("constante", `Null)
    ...
    | Shared.Eval_tree.Unary_op ((Is_undef, pos), value) ->
        let id = Utils.Pos.to_id pos in
        ( "operation unaire"
        , `O
            [ ("type", `String "is undefined")
            ; ("id", `String id)
            ; ("valeur", to_value value) ] )
...
let to_rules ((name, value) : Shared.Rule_name.t * Hashed_tree.value) =
  (Shared.Rule_name.show name, to_value value)

let to_yaml (ast : Hashed_tree.t) : Yaml.value =
  let rules = Hashtbl.to_alist ast |> List.map ~f:to_rules in
  `O rules

let to_string (ast : Hashed_tree.t) : string = Yaml.to_string_exn @@ to_yaml ast
