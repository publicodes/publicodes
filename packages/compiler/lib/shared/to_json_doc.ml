...
let to_mapping
  ...
  let rec to_value_mec (value_meca, pos) : Yojson.Basic.t =
    ...
    in
    let _type = [("type", `String _type)] in
    `Assoc (_type @ [to_publicodes pos] @ parameters)
  and to_chain_mec (chain_meca, pos) : Yojson.Basic.t =
    let (_type : string), (parameters : Yojson.Basic.t) =
      match chain_meca with
      | Context contexts ->
          let contexts : Yojson.Basic.t list =
            List.map contexts ~f:(function (key, pos), value ->
                `Assoc
                  [ ("name", `String (Rule_name.show key))
                  ; to_publicodes pos
                  ; ("value", `Assoc (to_mecs value)) ] )
          in
