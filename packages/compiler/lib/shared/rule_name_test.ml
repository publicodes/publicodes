...
let%test_unit "do not disambiguate complete reference" =
  let rule_names = Rule_name.Set.of_list [mk ["rule 1"]; mk ["rule 2"]] in
  let current = mk ["rule 1"] in
  let ref = ["rule 2"] in
  [%test_eq: t option] (resolve ~rule_names ~current ref) (Some (mk ref)) ;
  let rule_names = Rule_name.Set.of_list [mk ["a"]; mk ["a"; "b"]] in
  let current = mk ["a"] in
  let ref = ["a"; "b"] in
  [%test_eq: t option] (resolve ~rule_names ~current ref) (Some (mk ref))
...
let%test_unit "disambiguate child rule" =
  ...
  let rule_names = Rule_name.Set.of_list [mk ["a"]; mk ["a"; "b"; "c"]] in
  let current = mk ["a"] in
  [%test_eq: t option]
...
let%test_unit "disambiguate parent rule" =
  let rule_names =
    Rule_name.Set.of_list [mk ["a"]; mk ["a"; "b"]; mk ["a"; "b"; "c"]]
  in
...
let%test_unit "disambiguate child rule first" =
  let rule_names =
    Rule_name.Set.of_list [mk ["a"]; mk ["a"; "a"]; mk ["a"; "a"; "a"]]
  in
