...
and parse_import ~default_to_public ~ctx mapping =
  ...
  | Some (import, pos) -> (
      let* package, (module_, pos) =
        ...
        | `O mapping -> (
            let* module_ =
              match find_value "module" mapping with
              | None ->
                  let code, message = Err.parsing_invalid_import in
                  fatal_error ~pos ~code ~kind:`Syntax message
              | Some (module_, _) ->
                  let* value = get_scalar ~pos module_ >>| get_value in
                  return (value, pos)
            in
            ...
            | Some (package, pos) -> (
                let* value = get_scalar ~pos package >>| get_value in
