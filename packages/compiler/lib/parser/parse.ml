...
and parse_import ~default_to_public ~ctx mapping =
  ...
  | Some (import, pos) -> (
      let* package, (module_, pos) =
        ...
        | `O mapping -> (
            ...
            | Some (package, pos) -> (
                ...
                let* package =
                  if Utils.File.is_valid value then return value
                  else
                    let code, message = Err.invalid_path in
                    fatal_error ~pos ~code ~kind:`Syntax message
                      ~hints:
                        [ Stdlib.Format.sprintf
                            "'%s' n'est pas une valeur de paquet valide" value
                        ]
                in
                match
                  Utils.File.publicodes_package ctx.current_package package
                with
                | Error (Not_found paths) ->
                    ...
                    fatal_error ~pos ~code ~kind:`Syntax message
                      ~hints:
                        [ Stdlib.Format.sprintf
                            "le dossier n'a pas été trouvé dans aucun des \
                             emplacements suivants : %s"
                            paths ]
                | Error Absent_env ->
                    ...
                    fatal_error ~pos ~code ~kind:`Syntax message
                      ~hints:
                        [ Stdlib.Format.sprintf
                            "la variable d'environnement PUBLICODESPATH n'est \
                             pas configurée" ]
                | Error (Invalid_env parts) ->
                    ...
                    fatal_error ~pos ~code ~kind:`Syntax message
                      ~hints:
                        [ Stdlib.Format.sprintf
                            "certains composants de PUBLICODESPATH ne sont pas \
                             valide : %s"
                            parts ]
                | Error _ ->
      ...
      let* module_ =
        ...
        if Utils.File.is_valid module_ then return module_
        else
          let code, message = Err.invalid_path in
          fatal_error ~pos ~code ~kind:`Syntax message
            ~hints:
