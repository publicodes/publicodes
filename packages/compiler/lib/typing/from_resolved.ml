...
let rec transform_expr (expr : Shared_ast.resolved_expr) : Ast.wip_expr =
  let expr, {Mark.pos} = expr in
  match expr with
  | Const (Number (number, unit)) as expr ->
      (expr, Ast.mk_lit_number ~pos number unit)
  | Const (Bool bool) as expr ->
      (expr, Ast.mk_lit_bool ~pos bool)
  | Const (String string) as expr ->
      (expr, Ast.mk_lit_string ~pos string)
  | Const (Symbol symbol) as expr ->
      (expr, Ast.mk_lit_symbol ~pos symbol)
  | Const (Date (Day {day; year; month})) as expr ->
      (expr, Ast.mk_lit_day ~pos day year month)
  | Const (Date (Month {year; month})) as expr ->
      (expr, Ast.mk_lit_month ~pos year month)
  | Ref _ as expr ->
      (expr, Ast.mk_any ~pos)
  | Binary_op (op, left, right) ->
      ...
      let mark =
        ...
        | _ ->
            Ast.mk_any ~pos
      in
      (Binary_op (op, left, right), mark)
  | Unary_op (op, value) ->
      let value = transform_expr value in
      (Unary_op (op, value), Ast.mk_any ~pos)
...
and transform_value (value : Shared_ast.resolved_value) : Ast.wip_value =
  let {Shared_ast.value; chainable_mechanisms}, {Mark.pos} = value in
  let value =
    let value, {Mark.pos} = value in
    let value = transform_value_mechanism value in
    (value, Ast.mk_any ~pos)
  in
  let chainable_mechanisms =
    List.map chainable_mechanisms ~f:(fun chainable ->
        let chainable, {Mark.pos} = chainable in
        let chainable = transform_chainable_mechanism chainable in
        (chainable, Ast.mk_any ~pos) )
  in
  ({Shared_ast.value; chainable_mechanisms}, Ast.mk_any ~pos)
