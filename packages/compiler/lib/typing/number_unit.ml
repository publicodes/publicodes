...
let to_concrete (list : t) : Units.t =
  let normalized = normalize list in
  normalized.concrete
  ...
  | ( {concrete= concrete1; elem= []; inv= []}
    , {concrete= concrete2; elem= []; inv= []} ) ->
      ...
      if Units.equal concrete1 concrete2 |> not then
        ...
        let labels =
          [ Mark.mk_pos ~pos:pos1
              (Stdlib.Format.asprintf "unité: %a" Units.pp concrete1)
          ; Mark.mk_pos ~pos:pos2
              (Stdlib.Format.asprintf "unité: %a" Units.pp concrete2) ]
          |> List.sort ~compare:(fun (_, {Mark.pos= p1}) (_, {Mark.pos= p2}) ->
              Pos.compare p1 p2 )
        in
        fatal_error ~pos:pos1 ~kind:`Type ~code ~labels message
      else return ()
...
let compare (u1 : t) (u2 : t) = String.compare (to_string u1) (to_string u2)

let is_any (u : t) =
  List.exists u ~f:(fun elem ->
      match UnionFind.get elem with Variable _ -> true | _ -> false )
