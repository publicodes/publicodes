
/** Compiled private Publicodes rules */

{%- macro fmt_binop (op) -%}
	{%- switch op -%}
	{%- case "add" -%} $add
	{%- case "sub" -%} $sub
	{%- case "mul" -%} $mul
	{%- case "div" -%} $div
	{%- case "pow" -%} $pow
	{%- case "eq" -%} $eq
	{%- case "neq" -%} $neq
	{%- case "lt" -%} $lt
	{%- case "gt" -%} $gt
	{%- case "gte" -%} $gte
	{%- case "lte" -%} $lte
	{%- case "and" -%} $and
	{%- case "or" -%} $or
	{%- case "min" -%} $min
	{%- case "max" -%} $max
	{%- endswitch %}
{%- endmacro %}

{%- macro fmt_identifier (name) -%}
	{{ name
		| replace("_", "___")
		| replace(" ", "_")
		| replace("\\.", "·")
		| replace("'", "ʹ")
		| replace("-", "__t__")
		| replace("«", "__go__")
		| replace("»", "__gf__")
		| replace("\"", "__d__")
		| replace("€", "__euro__")
		| replace("%", "__pct__")
		| replace("²", "__sq__")
	}}
{%- endmacro -%}

{%- macro fmt_method (name) -%}
	_{{ fmt_identifier(name) }}
{%- endmacro -%}

{%- macro fmt_type (name) -%}
	{{ fmt_identifier(name) }}Params
{%- endmacro -%}

{%- macro fmt_ts_type (type) -%}
	{%- switch type %}
	{%- case "text" -%}
		string
	{%- case "date" -%}
		Date
	{%- default -%}
		{{ type }}
	{%- endswitch %}
{%- endmacro -%}

{%- macro fmt_esc (input) -%}
	{{ input
		| replace ("'", "\\'")
		| replace ("\n", "\\n")
		| replace ("\t", "\\t") }}
{%- endmacro -%}

{%- macro indent (prefix) -%}
{%- set input -%}
{{ caller() }}
{%- endset -%}
{{ replace ("\n", "\n" + prefix, input) | replace(" +$", "") }}
{%- endmacro -%}

{%- macro indentret (prefix) -%}
{%- set input %}
{{ caller() }}
{%- endset -%}
{{ replace ("\n", "\n" + prefix, input) | replace(" +$", "") }}
{%- endmacro -%}

{%- macro fmt_rule (rule) -%}
	{%- set rule_type, rule_value, rule_id = (rule.rule_type, rule.rule_value, rule.rule_id) -%}
	$ret("{{ rule_id }}", ctx, {% switch rule_type %}
	{%- case "number" -%}
		{{ rule_value.number }}
	{%- case "text" -%}
		"{{ fmt_esc(rule_value) }}"
	{%- case "bool" -%}
		{{ rule_value }}
	{%- case "date" -%}
		{%- if rule_value.day -%}
			{{ printf("new Date('%d-%02d-%02d')",
				rule_value.year, rule_value.month, rule_value.day) }}
		{%- else -%}
			{{ printf("new Date('%02d-%02d')",
				rule_value.year, rule_value.month) }}
		{%- endif -%}
	{%- case "null" -%}
		null
	{%- case "undefined" -%}
		undefined
	{%- case "round" -%}
		$round("{{ rule_value.mode }}", {{ fmt_rule(rule_value.number) }}, () => {{ fmt_rule(rule_value.precision) }})
	{%- case "condition" -%}
		$cond(
			{%- call indentret ("  ") -%}
			{{ fmt_rule(rule_value.cond) }}, () => {{ fmt_rule(rule_value.then) }}, () => {{ fmt_rule(rule_value._else) }}
			{%- endcall -%}
		)
	{%- case "binary_op" -%}
		{{ fmt_binop(rule_value.op) }}(
			{%- call indentret ("  ") -%}
			{{ fmt_rule(rule_value.left) }},
			{%- endcall -%}
			{%- call indentret ("  ") -%}
				{%- if rule_value.lazy -%}
				() => {{ fmt_rule(rule_value.right) }}
				{%- else -%}
				{{ fmt_rule(rule_value.right) }}
				{%- endif -%}
			{%- endcall -%}
		)
	{%- case "unary_op" -%}
		{%- switch rule_value.op -%}
		{%- case "neg_op" -%}
			(-{{ fmt_rule(rule_value.arg) }})
		{%- case "is_undef" -%}
			({{ fmt_rule(rule_value.arg) }} === undefined)
		{%- endswitch -%}
	{%- case "ref" -%}
		$ref("{{ rule_value }}", {{ fmt_method(rule_value) }}, ctx, params)
	{%- case "get_ctx" -%}
		$get("{{ rule_value }}", ctx, params)
	{%- case "set_ctx" -%}
		((ctx) => {{ fmt_rule(rule_value.expr) }})(
			{
				...ctx,
				{%- for item in rule_value.items %}
				{%- set item_name, item_value = (item.name, item.value) %}
					"{{ item_name }}": {{ fmt_rule(item_value) }},
				{% endfor %}
			}
		)
	{%- default -%}
		not handled {{ rule_type }}
	{%- endswitch -%}
	)
{%- endmacro %}

{%- for rule_type, rule_name, rule_data in rules %}

/** @type {Fn<{{ fmt_ts_type(rule_type) }}>} */
function {{ fmt_method(rule_name) }}(ctx, params) {
  return /** @type \{{{ fmt_ts_type(rule_type)}}\} */ (
    {%- call indentret ("    ") -%}
    {{ fmt_rule(rule_data) }}
    {%- endcall %}
  )
}
{%- endfor %}

/** Exported outputs/inputs */

const rules = {
  {%- for out in outputs %}
  {%- if out.title || out.description %}
  /**
   {%- if out.title %}
   * **{{ out.title }}**
   {%- endif %}
   {%- if out.description %}
   {%- if out.title %}
   *
   {%- endif %}
   * {% call indent ("   * ") -%}
     {{ trim (out.description) }}
   {%- endcall %}
   {%- endif %}
   */
  {%- endif %}
  '{{ fmt_esc(out.rule_name) }}': {
    /**
     * Parameters of "{{ out.rule_name }}"
     * @typedef \{{
     {%- for param_type, param_value in out.params %}
     *  '{{ fmt_esc(param_value) }}'?: {{ fmt_ts_type(param_type) }} | undefined
     {%- endfor %}
     * }} {{ fmt_identifier(out.rule_name) }}Params
     */
    /**
     * Evaluate "{{ out.rule_name }}"
     * @type {(params?: {{ fmt_type(out.rule_name) }}, options?: {cache?: boolean}) => {{ fmt_ts_type(out.return_type) }} | undefined | null}
     */
    evaluate: (params = {}, options) =>
      $evaluate({{ fmt_method(out.rule_name) }}, params, options).value,
    /**
     * Evaluate "{{ out.rule_name }}" with evaluation trace, and information on
     * missing and needed parameters.
     * @type {(params?: {{ fmt_type(out.rule_name) }}, options?: {cache?: boolean}) => {value: {{ fmt_ts_type(out.return_type) }} | undefined | null, needed: Array<keyof {{ fmt_type(out.rule_name) }}>, missing: Array<keyof {{ fmt_type(out.rule_name) }}>}}
     */
    evaluateParams: (params = {}, options) =>
      $evaluate({{ fmt_method(out.rule_name) }}, params, options),
    /** @type {"{{ out.rule_type }}"} */
    type: "{{ out.rule_type }}",
    {%- if out.unit %}
    /** @type {"{{ out.unit }}"} */
    unit: "{{ out.unit }}",
    {%- endif %}
    /**
     * Parameter list for "{{ out.rule_name }}"
     * @type {Array<keyof {{ fmt_type(out.rule_name) }}>}
     */
    params: [
    {%- call indent ("             ") -%}
    {%- for param_type, param_value in out.params -%}
      '{{ fmt_esc(param_value) }}'{% if !loop.last %},{{ "\n" }}{% endif %}
    {%- endfor -%}
    {%- endcall -%}
    ],
    {%- for meta_type, meta_value in out.metas %}
    {%- switch meta_type %}
    {%- case "custom" %}
    /** Custom meta of rule "{{ out.rule_name }}" */
    meta: {{ meta_value }} /** @type {const} */,
    {%- default %}
    /** @type {string} */
    {{ meta_type }}: '{{ fmt_esc(meta_value) }}',
    {%- endswitch %}
    {%- endfor %}
  }{% if !loop.last %},{% endif %}
  {%- endfor %}
}

export default rules;
