...
var require_identity = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/nodes/identity.js': (exports) => {
		...
		const isScalar$1 = (node) =>
			!!node && typeof node === 'object' && node[NODE_TYPE] === SCALAR$1
		const isSeq = (node) =>
		...
		const hasAnchor = (node) =>
			(isScalar$1(node) || isCollection$1(node)) && !!node.anchor
		exports.ALIAS = ALIAS
		exports.DOC = DOC
		exports.MAP = MAP
...
var require_visit = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/visit.js': (exports) => {
		...
		function visit$5(node, visitor) {
			...
			if (identity$32.isDocument(node)) {
				if (
					visit_(null, node.contents, visitor_, Object.freeze([node])) ===
					REMOVE$1
				)
					node.contents = null
		...
		function replaceNode(key, path, node) {
			...
			else if (identity$32.isPair(parent))
...
var require_directives = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/doc/directives.js': (exports) => {
		...
		var Directives = class Directives {
			...
			toString(doc) {
				const lines =
					this.yaml.explicit ? [`%YAML ${this.yaml.version || '1.2'}`] : []
...
var require_anchors = /* @__PURE__ */ __commonJS({
				...
				setAnchors: () => {
					for (const source of aliasObjects) {
						...
						if (
							typeof ref === 'object' &&
							ref.anchor &&
							(identity$30.isScalar(ref.node) ||
								identity$30.isCollection(ref.node))
						)
...
var require_applyReviver = /* @__PURE__ */ __commonJS({
...
var require_toJS = /* @__PURE__ */ __commonJS({
		...
		var NodeBase = class {
			...
			toJS(doc, { mapAsMap, maxAliasCount, onAnchor, reviver } = {}) {
				...
				if (typeof onAnchor === 'function')
					for (const { count, res: res$1 } of ctx.anchors.values())
						onAnchor(res$1, count)
...
var require_Alias = /* @__PURE__ */ __commonJS({
			...
			toJSON(_arg, ctx) {
				...
				if (maxAliasCount >= 0) {
					...
					if (data.aliasCount === 0)
						data.aliasCount = getAliasCount(doc, source, anchors$3)
...
var require_Scalar = /* @__PURE__ */ __commonJS({
...
var require_createNode = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/doc/createNode.js': (exports) => {
		...
		function createNode$4(value, tagName, ctx) {
			...
			if (identity$25.isPair(value)) {
				const map$6 = ctx.schema[identity$25.MAP].createNode?.(
					ctx.schema,
					null,
					ctx,
				)
			...
			if (!tagObj) {
				...
				tagObj =
					value instanceof Map ? schema$6[identity$25.MAP]
					: Symbol.iterator in Object(value) ? schema$6[identity$25.SEQ]
					: schema$6[identity$25.MAP]
			}
			if (onTagObj) {
				onTagObj(tagObj)
				delete ctx.onTagObj
			...
			const node =
				tagObj?.createNode ? tagObj.createNode(ctx.schema, value, ctx)
				: typeof tagObj?.nodeClass?.from === 'function' ?
					tagObj.nodeClass.from(ctx.schema, value, ctx)
				:	new Scalar$19.Scalar(value)
...
var require_Collection = /* @__PURE__ */ __commonJS({
		...
		var Collection$3 = class extends Node.NodeBase {
			...
			clone(schema$6) {
				...
				copy.items = copy.items.map((it) =>
					identity$24.isNode(it) || identity$24.isPair(it) ?
						it.clone(schema$6)
					:	it,
				)
				if (this.range) copy.range = this.range.slice()
			...
			getIn(path, keepScalar) {
				...
				const node = this.get(key, true)
				if (rest.length === 0)
					return !keepScalar && identity$24.isScalar(node) ? node.value : node
				else
					return identity$24.isCollection(node) ?
...
var require_stringifyComment = /* @__PURE__ */ __commonJS({
...
var require_foldFlowLines = /* @__PURE__ */ __commonJS({
...
var require_stringifyString = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/stringify/stringifyString.js': (exports) => {
		...
		function blockString(
			...
			const literal =
				blockQuote === 'literal' ? true
				: blockQuote === 'folded' || type === Scalar$18.Scalar.BLOCK_FOLDED ?
					false
				: type === Scalar$18.Scalar.BLOCK_LITERAL ? true
			...
			if (!literal) {
				...
				if (blockQuote !== 'folded' && type !== Scalar$18.Scalar.BLOCK_FOLDED)
		...
		function plainString(item, ctx, onComment, onChompKeep) {
			...
			if (actualString) {
				...
				if (tags$1.some(test) || compat?.some(test))
					return quotedString(value, ctx)
			}
			return implicitKey ? str : (
					foldFlowLines.foldFlowLines(
		...
		function stringifyString$4(item, ctx, onComment, onChompKeep) {
			...
			if (type !== Scalar$18.Scalar.QUOTE_DOUBLE) {
				if (/[\x00-\x08\x0b-\x1f\x7f-\x9f\u{D800}-\u{DFFF}]/u.test(ss.value))
					type = Scalar$18.Scalar.QUOTE_DOUBLE
			}
			const _stringify = (_type) => {
				switch (_type) {
					case Scalar$18.Scalar.BLOCK_FOLDED:
					case Scalar$18.Scalar.BLOCK_LITERAL:
						return implicitKey || inFlow ?
								quotedString(ss.value, ctx)
							:	blockString(ss, ctx, onComment, onChompKeep)
					case Scalar$18.Scalar.QUOTE_DOUBLE:
						return doubleQuotedString(ss.value, ctx)
					case Scalar$18.Scalar.QUOTE_SINGLE:
						return singleQuotedString(ss.value, ctx)
					case Scalar$18.Scalar.PLAIN:
...
var require_stringify = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/stringify/stringify.js': (exports) => {
		...
		function stringifyProps(node, tagObj, { anchors: anchors$1$1, doc }) {
			...
			const props = []
			const anchor =
				(identity$23.isScalar(node) || identity$23.isCollection(node)) &&
				node.anchor
...
var require_stringifyPair = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/stringify/stringifyPair.js': (exports) => {
		...
		function stringifyPair$1({ key, value }, ctx, onComment, onChompKeep) {
			...
			if (simpleKeys) {
				...
				if (
					identity$22.isCollection(key) ||
					(!identity$22.isNode(key) && typeof key === 'object')
				)
					throw new Error(
			...
			let explicitKey =
				...
				(!key ||
					...
					identity$22.isCollection(key) ||
					(identity$22.isScalar(key) ?
						key.type === Scalar$17.Scalar.BLOCK_FOLDED ||
						key.type === Scalar$17.Scalar.BLOCK_LITERAL
					:	typeof key === 'object'))
			...
			if (ctx.inFlow) {
				...
				if (keyComment && !keyCommentDone)
					str += stringifyComment$2.lineComment(
						str,
						ctx.indent,
			...
			if (!explicitKey && !keyComment && identity$22.isScalar(value))
			...
			const valueStr = stringify$5.stringify(
				value,
				ctx,
...
var require_log = /* @__PURE__ */ __commonJS({
...
var require_merge = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/schema/yaml-1.1/merge.js': (exports) => {
		...
		const merge$3 = {
			...
			test: /^<<$/,
			resolve: () =>
				Object.assign(new Scalar$16.Scalar(Symbol(MERGE_KEY)), {
					addToJSMap: addMergeToJSMap,
				}),
			stringify: () => MERGE_KEY,
		}
		const isMergeKey = (ctx, key) =>
			(merge$3.identify(key) ||
				(identity$21.isScalar(key) &&
					(!key.type || key.type === Scalar$16.Scalar.PLAIN) &&
		...
		function addMergeToJSMap(ctx, map$6, value) {
			...
			if (identity$21.isSeq(value))
				for (const it of value.items) mergeValue(ctx, map$6, it)
			else if (Array.isArray(value))
				for (const it of value) mergeValue(ctx, map$6, it)
			else mergeValue(ctx, map$6, value)
		}
		function mergeValue(ctx, map$6, value) {
			const source =
				ctx && identity$21.isAlias(value) ? value.resolve(ctx.doc) : value
			if (!identity$21.isMap(source))
				throw new Error('Merge sources must be maps or map aliases')
			const srcMap = source.toJSON(null, ctx, Map)
			for (const [key, value$1] of srcMap)
				if (map$6 instanceof Map) {
					if (!map$6.has(key)) map$6.set(key, value$1)
				} else if (map$6 instanceof Set) map$6.add(key)
				else if (!Object.prototype.hasOwnProperty.call(map$6, key))
					Object.defineProperty(map$6, key, {
						value: value$1,
						writable: true,
						enumerable: true,
						configurable: true,
...
var require_addPairToJSMap = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/nodes/addPairToJSMap.js': (exports) => {
		...
		function addPairToJSMap$2(ctx, map$6, { key, value }) {
			if (identity$20.isNode(key) && key.addToJSMap)
				key.addToJSMap(ctx, map$6, value)
			else if (merge$2.isMergeKey(ctx, key))
				merge$2.addMergeToJSMap(ctx, map$6, value)
			else {
				const jsKey = toJS$3.toJS(key, '', ctx)
				if (map$6 instanceof Map)
					map$6.set(jsKey, toJS$3.toJS(value, jsKey, ctx))
				else if (map$6 instanceof Set) map$6.add(jsKey)
				else {
					const stringKey = stringifyKey(key, jsKey, ctx)
					const jsValue = toJS$3.toJS(value, stringKey, ctx)
					if (stringKey in map$6)
						Object.defineProperty(map$6, stringKey, {
							value: jsValue,
							writable: true,
							enumerable: true,
							configurable: true,
...
var require_Pair = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/nodes/Pair.js': (exports) => {
		...
		var Pair$7 = class Pair$7 {
			constructor(key, value = null) {
				Object.defineProperty(this, identity$19.NODE_TYPE, {
					value: identity$19.PAIR,
				})
				this.key = key
				this.value = value
			}
...
var require_stringifyCollection = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/stringify/stringifyCollection.js': (
		...
		function stringifyCollection$2(collection, ctx, options) {
			return (
				(ctx.inFlow ?? collection.flow) ?
					stringifyFlowCollection
				:	stringifyBlockCollection)(collection, ctx, options)
		}
		function stringifyBlockCollection(
			...
			for (let i = 0; i < items.length; ++i) {
				...
				chompKeep = false
				let str$1 = stringify$3.stringify(
					item,
					itemCtx,
					() => (comment$1 = null),
					() => (chompKeep = true),
				...
				if (comment$1)
					str$1 += stringifyComment$1.lineComment(
						str$1,
						itemIndent,
		...
		function stringifyFlowCollection(
			...
			for (let i = 0; i < items.length; ++i) {
				...
				if (comment)
					str += stringifyComment$1.lineComment(
						str,
						itemIndent,
		...
		function addCommentBefore(
			...
			if (comment) {
				const ic = stringifyComment$1.indentComment(
					commentString(comment),
					indent,
				)
...
var require_YAMLMap = /* @__PURE__ */ __commonJS({
		...
		var YAMLMap$8 = class extends Collection$2.Collection {
			...
			static from(schema$6, obj, ctx) {
				...
				const add = (key, value) => {
					...
					if (value !== void 0 || keepUndefined)
						map$6.items.push(Pair$6.createPair(key, value, ctx))
				}
				if (obj instanceof Map) for (const [key, value] of obj) add(key, value)
				else if (obj && typeof obj === 'object')
					for (const key of Object.keys(obj)) add(key, obj[key])
				if (typeof schema$6.sortMapEntries === 'function')
					map$6.items.sort(schema$6.sortMapEntries)
				return map$6
			}
			/**
			...
			add(pair, overwrite) {
				...
				if (prev) {
					...
					if (
						identity$17.isScalar(prev.value) &&
						Scalar$15.isScalarValue(_pair.value)
					)
			...
			get(key, keepScalar) {
				...
				return (
					(!keepScalar && identity$17.isScalar(node) ? node.value : node) ??
					void 0
				)
			}
			has(key) {
			...
			toJSON(_, ctx, Type) {
				...
				for (const item of this.items)
					addPairToJSMap.addPairToJSMap(ctx, map$6, item)
				return map$6
			}
			toString(ctx, onComment, onChompKeep) {
				if (!ctx) return JSON.stringify(this)
				for (const item of this.items)
					if (!identity$17.isPair(item))
						throw new Error(
...
var require_map = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/schema/common/map.js': (exports) => {
		...
		const map$5 = {
			...
			resolve(map$6, onError) {
				if (!identity$16.isMap(map$6))
					onError('Expected a mapping for this tag')
				return map$6
			},
			createNode: (schema$6, obj, ctx) =>
				YAMLMap$7.YAMLMap.from(schema$6, obj, ctx),
		}
		exports.map = map$5
	},
})
...
var require_YAMLSeq = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/nodes/YAMLSeq.js': (exports) => {
		...
		var YAMLSeq$7 = class extends Collection$1.Collection {
			...
			set(key, value) {
				...
				if (identity$15.isScalar(prev) && Scalar$14.isScalarValue(value))
					prev.value = value
			...
			toJSON(_, ctx) {
				...
				let i = 0
				for (const item of this.items)
					seq$6.push(toJS$2.toJS(item, String(i++), ctx))
				return seq$6
			}
			toString(ctx, onComment, onChompKeep) {
				if (!ctx) return JSON.stringify(this)
...
var require_seq = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/schema/common/seq.js': (exports) => {
		...
		const seq$5 = {
			...
			resolve(seq$6, onError) {
				if (!identity$14.isSeq(seq$6))
					onError('Expected a sequence for this tag')
				return seq$6
			},
			createNode: (schema$6, obj, ctx) =>
				YAMLSeq$6.YAMLSeq.from(schema$6, obj, ctx),
		}
		exports.seq = seq$5
	},
})
...
var require_string = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/schema/common/string.js': (exports) => {
		...
		const string$4 = {
			...
			stringify(item, ctx, onComment, onChompKeep) {
				...
				return stringifyString$2.stringifyString(
					item,
					ctx,
					onComment,
					onChompKeep,
...
var require_null = /* @__PURE__ */ __commonJS({
...
var require_bool$1 = /* @__PURE__ */ __commonJS({
...
var require_stringifyNumber = /* @__PURE__ */ __commonJS({
...
var require_float$1 = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/schema/core/float.js': (exports) => {
		...
		const floatNaN$1 = {
			...
			resolve: (str) =>
				str.slice(-3).toLowerCase() === 'nan' ? NaN
		...
		const floatExp$1 = {
			...
			stringify(node) {
				...
				return isFinite(num) ?
						num.toExponential()
					:	stringifyNumber$4.stringifyNumber(node)
			},
		}
		const float$4 = {
			identify: (value) => typeof value === 'number',
...
var require_int$1 = /* @__PURE__ */ __commonJS({
...
var require_schema$2 = /* @__PURE__ */ __commonJS({
...
var require_schema$1 = /* @__PURE__ */ __commonJS({
...
var require_pairs = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/schema/yaml-1.1/pairs.js': (exports) => {
		...
		function resolvePairs(seq$6, onError) {
			if (identity$13.isSeq(seq$6))
				for (let i = 0; i < seq$6.items.length; ++i) {
					...
					if (identity$13.isPair(item)) continue
						...
						const pair =
							item.items[0] || new Pair$5.Pair(new Scalar$8.Scalar(null))
					...
					seq$6.items[i] =
						identity$13.isPair(item) ? item : new Pair$5.Pair(item)
				}
...
var require_omap = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/schema/yaml-1.1/omap.js': (exports) => {
		...
		const omap$2 = {
			...
			resolve(seq$6, onError) {
				...
				const seenKeys = []
				for (const { key } of pairs$1$1.items)
					if (identity$12.isScalar(key))
			...
			createNode: (schema$6, iterable, ctx) =>
				YAMLOMap.from(schema$6, iterable, ctx),
		}
...
var require_bool = /* @__PURE__ */ __commonJS({
...
var require_float = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/schema/yaml-1.1/float.js': (exports) => {
		...
		const floatExp = {
			...
			stringify(node) {
				...
				return isFinite(num) ?
						num.toExponential()
					:	stringifyNumber$2.stringifyNumber(node)
			},
		}
		const float$2 = {
			identify: (value) => typeof value === 'number',
...
var require_int = /* @__PURE__ */ __commonJS({
...
var require_set = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/schema/yaml-1.1/set.js': (exports) => {
		...
		var YAMLSet = class YAMLSet extends YAMLMap$5.YAMLMap {
			...
			get(key, keepPair) {
				...
				return (
					!keepPair && identity$11.isPair(pair) ?
						identity$11.isScalar(pair.key) ?
							pair.key.value
						:	pair.key
					:	pair
				)
		...
		const set$2 = {
			...
			createNode: (schema$6, iterable, ctx) =>
				YAMLSet.from(schema$6, iterable, ctx),
			resolve(map$6, onError) {
				if (identity$11.isMap(map$6))
					if (map$6.hasAllNullValues(true))
						return Object.assign(new YAMLSet(), map$6)
...
var require_timestamp = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/schema/yaml-1.1/timestamp.js': (exports) => {
		...
		function parseSexagesimal(str, asBigInt) {
			...
			const res = parts
				.replace(/_/g, '')
				.split(':')
				.reduce((res$1, p) => res$1 * num(60) + num(p), num(0))
...
var require_schema = /* @__PURE__ */ __commonJS({
...
var require_tags = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/schema/tags.js': (exports) => {
		...
		function getTags(customTags, schemaName, addMergeTag) {
			...
			if (!tags$1)
				if (Array.isArray(customTags)) tags$1 = []
				else {
			...
			if (Array.isArray(customTags))
				for (const tag of customTags) tags$1 = tags$1.concat(tag)
			else if (typeof customTags === 'function')
				tags$1 = customTags(tags$1.slice())
	...
	'../../node_modules/yaml/dist/schema/Schema.js': (exports) => {
		...
		var Schema$2 = class Schema$2 {
			constructor({
				...
				this.compat =
					Array.isArray(compat) ? tags.getTags(compat, 'compat')
					: compat ? tags.getTags(null, compat)
					: null
				...
				Object.defineProperty(this, identity$10.SCALAR, {
					value: string.string,
				})
				Object.defineProperty(this, identity$10.SEQ, { value: seq.seq })
				this.sortMapEntries =
					typeof sortMapEntries === 'function' ? sortMapEntries
					: sortMapEntries === true ? sortMapEntriesByKey
					: null
			}
			clone() {
				const copy = Object.create(
...
var require_stringifyDocument = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/stringify/stringifyDocument.js': (exports) => {
		...
		function stringifyDocument$1(doc, options) {
			...
			if (doc.contents) {
				...
				let body = stringify$2.stringify(
					doc.contents,
					ctx,
					() => (contentComment = null),
					onChompKeep,
	...
	'../../node_modules/yaml/dist/doc/Document.js': (exports) => {
		...
		var Document$4 = class Document$4 {
			constructor(value, replacer, options) {
				...
				this.warnings = []
				Object.defineProperty(this, identity$8.NODE_TYPE, {
					value: identity$8.DOC,
				})
				let _replacer = null
			...
			clone() {
				...
				copy.contents =
					identity$8.isNode(this.contents) ?
			 ...
			 */
			get(key, keepScalar) {
				return identity$8.isCollection(this.contents) ?
			...
			getIn(path, keepScalar) {
				if (Collection.isEmptyPath(path))
					return !keepScalar && identity$8.isScalar(this.contents) ?
							this.contents.value
						:	this.contents
				return identity$8.isCollection(this.contents) ?
...
var require_errors = /* @__PURE__ */ __commonJS({
...
var require_resolve_props = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/compose/resolve-props.js': (exports) => {
		function resolveProps$4(
			tokens,
...
var require_util_contains_newline = /* @__PURE__ */ __commonJS({
...
var require_util_flow_indent_check = /* @__PURE__ */ __commonJS({
...
var require_util_map_includes = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/compose/util-map-includes.js': (exports) => {
		...
		function mapIncludes(ctx, items, search) {
			...
			const isEqual =
				typeof uniqueKeys === 'function' ? uniqueKeys : (
					(a, b) =>
						a === b ||
						(identity$7.isScalar(a) &&
							identity$7.isScalar(b) &&
							a.value === b.value)
				)
...
var require_resolve_block_map = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/compose/resolve-block-map.js': (exports) => {
		...
		function resolveBlockMap$1(
			{ composeNode: composeNode$2, composeEmptyNode: composeEmptyNode$1 },
			ctx,
			bm,
			onError,
			tag,
			...
			for (const collItem of bm.items) {
				...
				if (implicitKey) {
					...
					if (!keyProps.anchor && !keyProps.tag && !sep) {
						...
						if (keyProps.comment)
							if (map$6.comment) map$6.comment += '\n' + keyProps.comment
							else map$6.comment = keyProps.comment
						continue
					}
					if (
						keyProps.newlineAfterProp ||
						utilContainsNewline$1.containsNewline(key)
					)
						onError(
				...
				const keyNode =
					key ?
						composeNode$2(ctx, key, keyProps, onError)
				...
				ctx.atKey = false
				if (utilMapIncludes$1.mapIncludes(ctx, map$6.items, keyNode))
				...
				if (valueProps.found) {
					...
					const valueNode =
						value ?
							composeNode$2(ctx, value, valueProps, onError)
						:	composeEmptyNode$1(ctx, offset, sep, null, valueProps, onError)
					if (ctx.schema.compat)
			...
			map$6.range = [bm.offset, offset, commentEnd ?? offset]
			return map$6
		}
...
var require_resolve_block_seq = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/compose/resolve-block-seq.js': (exports) => {
		...
		function resolveBlockSeq$1(
			...
			for (const { start, value } of bs.items) {
				...
				const node =
					value ?
						composeNode$2(ctx, value, props, onError)
					:	composeEmptyNode$1(ctx, props.end, start, null, props, onError)
				if (ctx.schema.compat)
					utilFlowIndentCheck.flowIndentCheck(bs.indent, value, onError)
				offset = node.range[2]
				seq$6.items.push(node)
			}
			seq$6.range = [bs.offset, offset, commentEnd ?? offset]
			return seq$6
		}
...
var require_resolve_end = /* @__PURE__ */ __commonJS({
...
var require_resolve_flow_collection = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/compose/resolve-flow-collection.js': (
		...
		function resolveFlowCollection$1(
			...
			const coll = new (tag?.nodeClass ??
				(isMap$2 ? YAMLMap$3.YAMLMap : YAMLSeq$2.YAMLSeq))(ctx.schema)
			coll.flow = true
			...
			for (let i = 0; i < fc.items.length; ++i) {
				...
				if (!isMap$2 && !sep && !props.found) {
					const valueNode =
						value ?
							composeNode$2(ctx, value, props, onError)
						:	composeEmptyNode$1(ctx, props.end, sep, null, props, onError)
					...
					const keyNode =
						key ?
							composeNode$2(ctx, key, props, onError)
						:	composeEmptyNode$1(ctx, keyStart, start, null, props, onError)
					...
					const valueNode =
						value ? composeNode$2(ctx, value, valueProps, onError)
						: valueProps.found ?
							composeEmptyNode$1(
								ctx,
								valueProps.end,
					...
					if (isMap$2) {
						const map$6 = coll
						if (utilMapIncludes.mapIncludes(ctx, map$6.items, keyNode))
							onError(keyStart, 'DUPLICATE_KEY', 'Map keys must be unique')
						map$6.items.push(pair)
					} else {
						...
						map$6.range = [keyNode.range[0], endRange[1], endRange[2]]
			...
			if (ee.length > 0) {
				const end = resolveEnd$4.resolveEnd(
					ee,
					cePos,
					ctx.options.strict,
					onError,
...
var require_compose_collection = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/compose/compose-collection.js': (exports) => {
		...
		function resolveCollection(CN$1, ctx, token, onError, tagName, tag) {
			const coll =
				token.type === 'block-map' ?
					resolveBlockMap.resolveBlockMap(CN$1, ctx, token, onError, tag)
				: token.type === 'block-seq' ?
					resolveBlockSeq.resolveBlockSeq(CN$1, ctx, token, onError, tag)
		...
		function composeCollection$1(CN$1, ctx, token, props, onError) {
			...
			const expType =
				token.type === 'block-map' ? 'map'
				: token.type === 'block-seq' ? 'seq'
				: token.start.source === '{' ? 'map'
				: 'seq'
			if (
				!tagToken ||
				!tagName ||
				tagName === '!' ||
				(tagName === YAMLMap$2.YAMLMap.tagName && expType === 'map') ||
				(tagName === YAMLSeq$1.YAMLSeq.tagName && expType === 'seq')
			)
...
var require_resolve_block_scalar = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/compose/resolve-block-scalar.js': (exports) => {
		...
		function resolveBlockScalar$2(ctx, scalar, onError) {
			...
			const type =
				header.mode === '>' ?
					Scalar$4.Scalar.BLOCK_FOLDED
				:	Scalar$4.Scalar.BLOCK_LITERAL
...
var require_resolve_flow_scalar = /* @__PURE__ */ __commonJS({
...
var require_compose_scalar = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/compose/compose-scalar.js': (exports) => {
		...
		function composeScalar$1(ctx, token, tagToken, onError) {
			const { value, type, comment, range } =
				...
				:	resolveFlowScalar$1.resolveFlowScalar(
						token,
						ctx.options.strict,
						onError,
					)
		...
		function findScalarTagByName(schema$6, value, tagName, tagToken, onError) {
			...
			const matchWithTest = []
			for (const tag of schema$6.tags)
		...
		function findScalarTagByTest(
			{ atKey, directives: directives$2, schema: schema$6 },
			value,
			token,
			onError,
		) {
			const tag =
				schema$6.tags.find(
					(tag$1) =>
						(tag$1.default === true || (atKey && tag$1.default === 'key')) &&
						tag$1.test?.test(value),
				) || schema$6[identity$4.SCALAR]
			if (schema$6.compat) {
				const compat =
					schema$6.compat.find(
						(tag$1) => tag$1.default && tag$1.test?.test(value),
					) ?? schema$6[identity$4.SCALAR]
...
var require_util_empty_scalar_position = /* @__PURE__ */ __commonJS({
...
var require_compose_node = /* @__PURE__ */ __commonJS({
...
var require_compose_doc = /* @__PURE__ */ __commonJS({
...
var require_composer = /* @__PURE__ */ __commonJS({
				...
				switch (token.type) {
					...
					case 'error': {
						const msg =
							...
							:	token.message
						const error = new errors$3.YAMLParseError(
							getErrorPos(token),
							'UNEXPECTED_TOKEN',
							msg,
						)
...
var require_cst_scalar = /* @__PURE__ */ __commonJS({
			...
			if (token) {
				const _onError = (pos, code, message) => {
					const offset =
						...
						: Array.isArray(pos) ? pos[0]
						: pos.offset
					if (onError) onError(offset, code, message)
					else
						throw new errors$2.YAMLParseError(
							[offset, offset + 1],
							code,
							message,
						)
...
var require_cst_stringify = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/parse/cst-stringify.js': (exports) => {
		 ...
		 */
		const stringify$1 = (cst$3) =>
			'type' in cst$3 ? stringifyToken(cst$3) : stringifyItem(cst$3)
...
var require_cst_visit = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/parse/cst-visit.js': (exports) => {
		...
		function visit$1(cst$3, visitor) {
			if ('type' in cst$3 && cst$3.type === 'document')
				cst$3 = {
					start: cst$3.start,
					value: cst$3.value,
				}
...
var require_cst = /* @__PURE__ */ __commonJS({
...
var require_lexer = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/parse/lexer.js': (exports) => {
		...
		var Lexer = class {
			...
			*parseBlockScalar() {
				...
				let indent = 0
				let ch
				loop: for (let i$1 = this.pos; (ch = this.buffer[i$1]); ++i$1)
					switch (ch) {
						case ' ':
							indent += 1
							break
				...
				if (ch === '	') {
					...
					do {
						...
						if (ch$1 === '\n' && i$1 >= this.pos && i$1 + 1 + indent > lastChar)
							nl = i$1
						else break
					} while (true)
				yield cst$2.SCALAR
...
var require_line_counter = /* @__PURE__ */ __commonJS({
...
var require_parser = /* @__PURE__ */ __commonJS({
				...
				switch (this.type) {
					case 'newline':
						...
						if (it.value) {
							...
							if (
								(Array.isArray(end) ? end[end.length - 1] : void 0)?.type ===
								'comment'
							)
				...
				switch (this.type) {
					...
					case 'seq-item-ind':
						...
						if (it.value || includesToken(it.start, 'seq-item-ind'))
							seq$6.items.push({ start: [this.sourceToken] })
						else it.start.push(this.sourceToken)
						return
				}
...
var require_public_api = /* @__PURE__ */ __commonJS({
	'../../node_modules/yaml/dist/public-api.js': (exports) => {
		...
		function parseOptions(options) {
			...
			return {
				lineCounter:
					options.lineCounter ||
					(prettyErrors && new lineCounter$1.LineCounter()) ||
					null,
				prettyErrors,
			}
		}
		...
		function parseDocument(source, options = {}) {
			...
			const composer$1$1 = new composer$1.Composer(options)
			let doc = null
			for (const _doc of composer$1$1.compose(
				parser$1$1.parse(source),
				true,
				source.length,
			))
				if (!doc) doc = _doc
		...
		function stringify(value, replacer, options) {
			...
			return new Document$1.Document(value, _replacer, options).toString(
				options,
			)
		}
		exports.parse = parse
	...
	import_dist.visit(doc, {
		Map(_, map$6) {
			if ((0, import_dist.isMap)(map$6)) {
				const formuleItem = map$6.items.find(
					(item) => item.key.value === 'formule',
				)
				if (formuleItem) {
...
function moveNonAuthorizedKeysToMeta(doc) {
	if ((0, import_dist.isMap)(doc.contents))
...
function processRuleMap(map$6) {
	...
	if (keysToMove.length > 0) {
		const metaMap =
			existingMeta && (0, import_dist.isMap)(existingMeta) ? existingMeta : (
	...
	if (avecValue && (0, import_dist.isMap)(avecValue))
