...
summary(() => {
	...
	bench('[Multiple engine comparison] Publicodes 2 (with cache)', () => {
		...
		;[(c1, c2)].forEach((c) =>
			testRules.forEach(({ rule }) =>
				do_not_optimize(newEngineAEWithCache.evaluate(rule, c)),
			),
		)
	})
	...
	bench('[Multiple engine comparison] Publicodes 2 (without cache)', () => {
		...
		;[c1, c2].forEach((c) =>
			testRules.forEach(({ rule }) =>
				newEngineAEWithoutCache.evaluate(rule, c),
			),
		)
	})
	...
	// 	;[(c1, c2)].forEach((c) =>
	// 		rules.forEach(({ rule }) =>
	// 			do_not_optimize(newEngineJSAEWithCache.evaluate(rule, c)),
	// 		),
	// 	)
	// })
	...
	bench('[Multiple engine comparison] Publicodes 2 JS', () => {
		const c1 = Object.assign({}, contexte)
		const c2 = Object.assign({}, contexte, changes)
		;[c1, c2].forEach((c) => testRules.forEach(({ rule }) => rules[rule](c)))
	})
})
