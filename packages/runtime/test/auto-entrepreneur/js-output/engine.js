export default class Engine {
	...
	rules = {
		...
		'dirigeant . auto-entrepreneur . cotisations et contributions . CFP': (
			...
			this.ref("entreprise . chiffre d'affaires . BIC", ctx) *
				(((
					(this.ref('entreprise . activité . nature', ctx) === 'artisanale') ===
						null ||
					(this.ref('entreprise . activité . nature', ctx) === 'artisanale') ===
						false
				) ?
			...
			(this.ref("entreprise . chiffre d'affaires . service BNC", ctx) *
				(((
					(this.ref('date', ctx) < new Date('2022-01') &&
						this.ref('dirigeant . auto-entrepreneur . Cipav', ctx) === false &&
						true) === null ||
					(this.ref('date', ctx) < new Date('2022-01') &&
						this.ref('dirigeant . auto-entrepreneur . Cipav', ctx) === false &&
						true) === false
		...
		'dirigeant . auto-entrepreneur . cotisations et contributions . cotisations . service BNC Cipav . répartition . autres contributions . taux de répartition':
			(ctx) =>
				(
					this.ref('date', ctx) >= new Date('2024-07') === null ||
					this.ref('date', ctx) >= new Date('2024-07') === false
				) ?
					36.3
				:	34.0,
		...
		'dirigeant . auto-entrepreneur . cotisations et contributions . cotisations . service BNC . répartition . invalidité-décès . taux de répartition':
			(ctx) =>
				(
					this.ref('date', ctx) >= new Date('2026-01') === null ||
					this.ref('date', ctx) >= new Date('2026-01') === false
				) ?
					(
						this.ref('date', ctx) >= new Date('2025-01') === null ||
						this.ref('date', ctx) >= new Date('2025-01') === false
					) ?
						(
							this.ref('date', ctx) >= new Date('2024-07') === null ||
							this.ref('date', ctx) >= new Date('2024-07') === false
		...
		'dirigeant . auto-entrepreneur . DROM . seconde période': (ctx) =>
			this.ref("entreprise . durée d'activité . années civiles", ctx) <= 3.0,
		...
		'dirigeant . auto-entrepreneur . Acre . taux Acre': (ctx) =>
			(
				...
				(
					...
					(
						this.ref("entreprise . durée d'activité", ctx) < 1.0 === null ||
						this.ref("entreprise . durée d'activité", ctx) < 1.0 === false
					) ?
						null
					:	50.0
				:	75.0
		...
		'dirigeant . auto-entrepreneur . DROM . première période': (ctx) =>
			this.ref("entreprise . durée d'activité . trimestres civils", ctx) <= 8.0,
				...
				(
					...
					(
						...
						(
							...
							(
								this.ref('date', ctx) >= new Date('2022-10') === null ||
								this.ref('date', ctx) >= new Date('2022-10') === false
							) ?
								22.0
							:	21.1
						:	23.1
		...
		'dirigeant . auto-entrepreneur . revenu net . après impôt': (ctx) =>
			this.ref('dirigeant . auto-entrepreneur . revenu net', ctx) -
		...
		'dirigeant . auto-entrepreneur . Cipav': (ctx) =>
			...
			(this.ref('entreprise . activité . nature', ctx) === 'libérale' &&
				this.ref('entreprise . date de création', ctx) < new Date('2018-01') &&
				this.ref('dirigeant . auto-entrepreneur . Cipav . adhérent', ctx) ===
					true &&
				true) ||
			false,
		...
		'dirigeant . auto-entrepreneur . cotisations et contributions . TFC . métiers . taux Moselle':
			(ctx) =>
				(
					...
					false
				:	this.ref('établissement . commune . département', ctx) === 'Moselle',
		...
		'dirigeant . auto-entrepreneur . Acre . taux CIPAV': (ctx) =>
			(
				this.ref('dirigeant . auto-entrepreneur . Cipav', ctx) === false ||
				this.ref('dirigeant . auto-entrepreneur . Cipav', ctx) === undefined
			) ?
				null
			: (
				this.ref('entreprise . date de création', ctx) >=
		...
		'dirigeant . auto-entrepreneur . Acre': (ctx) =>
			(
				this.ref('dirigeant . exonérations . ACRE', ctx) === false ||
