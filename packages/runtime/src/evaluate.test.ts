...
describe('evaluate', () => {
  test('should evaluate string references', () => {
    ...
    expect(evaluateNode(compiledPublicodes, value)).toBe(42)
  })
  ...
  test('should return null for null computations', () => {
    ...
    expect(evaluateNode(compiledPublicodes, value)).toBe(null)
  })
  ...
  test('should return undefined for empty arrays', () => {
    ...
    expect(evaluateNode(compiledPublicodes, value)).toBe(undefined)
  })

  test('should return the first element for single-element arrays', () => {
    const compiledPublicodes: CompiledPublicodes = {}

    expect(evaluateNode(compiledPublicodes, [42])).toBe(42)

    expect(evaluateNode(compiledPublicodes, ['hello'])).toBe('hello')
  })
  ...
  describe('Unary operations', () => {
    test('should handle unary negation operator', () => {
      ...
      expect(evaluateNode(compiledPublicodes, ['-', [5]])).toBe(-5)

      const withReference: CompiledPublicodes = {
        a: [10],
      }
      expect(evaluateNode(withReference, ['-', 'a'])).toBe(-10)
    })
    ...
    test('should handle nested operations', () => {
      ...
      expect(evaluateNode(compiledPublicodes, 'b')).toBe(-5)
    })
    ...
    test('should handle non-number values in unary operations', () => {
      ...
      expect(evaluateNode(compiledPublicodes, ['-', 'a'])).toBe(null)
      expect(evaluateNode(compiledPublicodes, ['-', 'b'])).toBe(undefined)
    })
  })

  describe('Binary operations', () => {
    test('should handle all arithmetic operator', () => {
      expect(evaluateNode({}, [[6], '+', [3]])).toBe(9)
      expect(evaluateNode({}, [[6], '-', [3]])).toBe(3)
      expect(evaluateNode({}, [[6], '*', [3]])).toBe(18)
      expect(evaluateNode({}, [[6], '/', [3]])).toBeCloseTo(2)
      expect(evaluateNode({}, [[6], '**', [3]])).toBe(216)
    })

    test('should handle all comparison operator', () => {
      expect(evaluateNode({}, [[6], '<', [3]])).toBe(false)
      expect(evaluateNode({}, [[6], '<=', [3]])).toBe(false)
      expect(evaluateNode({}, [[6], '<=', [6]])).toBe(true)

      expect(evaluateNode({}, [[6], '>', [3]])).toBe(true)
      expect(evaluateNode({}, [[6], '>=', [3]])).toBe(true)
      expect(evaluateNode({}, [[6], '>=', [6]])).toBe(true)
    ...
    test('should handle all logical operator', () => {
      expect(evaluateNode({}, [true, '&&', true])).toBe(true)
      expect(evaluateNode({}, [false, '||', false])).toBe(false)
      expect(evaluateNode({}, [false, '&&', true])).toBe(false)
      expect(evaluateNode({}, [true, '||', false])).toBe(true)
    })
    ...
    test('[multiplicative|comparison] should not evaluate the second operand when first operand is null', () => {
      ...
      operators.forEach((op) => {
        const run = () => evaluateNode(throwIfAccessed, [null, op, 'a'])
    ...
    test('[*|/|**] should not evaluate the second operand when first operand is 0', () => {
      ...
      operators.forEach((op) => {
        const run = () => evaluateNode(throwIfAccessed, [[0], op, 'a'])
        expect(run).not.toThrow()
        expect(run()).toBe(0)
      })
    })
    ...
    test('&& should not evaluate the second operand when first operand is false', () => {
      const run = () => evaluateNode(throwIfAccessed, [false, '&&', 'a'])
      expect(run).not.toThrow()
      expect(run()).toBe(false)
    })

    test('|| should not evaluate the second operand when first operand is true', () => {
      const run = () => evaluateNode(throwIfAccessed, [true, '||', 'a'])
      expect(run).not.toThrow()
    ...
    test('should return undefined if one operand is undefined', () => {
      ...
      operators.forEach((op) => {
        expect(evaluateNode({}, [[], op, [10]])).toBe(undefined)
      })
    })

    test('should not return undefined if we can deduce the result from the second operand', () => {
      expect(evaluateNode({}, [[], '&&', false])).toBe(false)
      expect(evaluateNode({}, [[], '||', true])).toBe(true)
      expect(evaluateNode({}, [[], '*', [0]])).toBe(0)
      expect(evaluateNode({}, [[], '**', [0]])).toBe(1)
      expect(() => evaluateNode({}, [[], '/', [0]])).toThrow('Division by zero')
    })
  ...
  describe('if-then-else', () => {
    describe('Conditional selection (ternary)', () => {
      ...
      test('should evaluate and return else branch if condition is false', () => {
        ...
        expect(evaluateNode(compiledPublicodes, value)).toBe(24)
      })
      ...
      test('should handle nested conditionals', () => {
        ...
        expect(evaluateNode(compiledPublicodes, value)).toBe(1)

        // Change outer condition to test nested evaluation
        compiledPublicodes.outerCondition = false
        expect(evaluateNode(compiledPublicodes, value)).toBe(3)
      })
    })
  })
  describe('date', () => {
    test('should evaluate date expressions', () => {
      expect(evaluateNode({}, { d: '2022-01-01' })).toEqual(
        new Date('2022-01-01').valueOf(),
      )
      expect(evaluateNode({}, { d: '2025-11' })).toEqual(
        new Date('2025-11-01').valueOf(),
      )
    })
    test('should allow date comparison', () => {
      expect(
        evaluateNode({}, [{ d: '2022-01-01' }, '<=', { d: '2022-01' }]),
      ).toEqual(true)
      expect(
        evaluateNode({}, [{ d: '2022-01-01' }, '<', { d: '2022-01-02' }]),
      ).toEqual(true)
      expect(
        evaluateNode({}, [{ d: '2022-01-02' }, '>', { d: '2022-01-01' }]),
      ).toEqual(true)
      expect(
        evaluateNode({}, [{ d: '2022-01-01' }, '>=', { d: '2022-01-01' }]),
      ).toEqual(true)
      expect(
        evaluateNode({}, [{ d: '2022-01-01' }, '=', { d: '2022-01-01' }]),
      ).toEqual(true)
