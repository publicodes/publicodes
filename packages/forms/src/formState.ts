...
import { splitByNamespace } from './pageSplitter'
...
export function initFormState<Name extends string>(
	...
	pageSplitter: (
		fields: Array<Name>,
	) => Array<Array<Name>> = splitByNamespace(),
	targets: Array<Name>,
) {
	...
	const formState: FormState<Name> = {
		targets,
		pages: [],
		currentPageIndex: -1,
		lastAnswered: null,
		nextPages: pageSplitter(computeNextFields(engine, { targets, pages: [] })),
	}
	return formState
}
...
export function handleInputChange<Name extends string>(
	{
		formState,
		engine,
		pageSplitter = splitByNamespace(),
	}: {
		formState: FormState<Name>
		engine: Engine<Name>
		pageSplitter: (fields: Array<Name>) => Array<Array<Name>>
	},
	dottedName: Name,
	...
	formState.pages = pageSplitter(computeNextFields(engine, formState))
